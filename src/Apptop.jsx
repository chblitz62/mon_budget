...
const BudgetTool = () => {
  ...
  const summary3Ans = [1, 2, 3].map(annee => {
    ...
    lieux.forEach(l => {
      ...
      detailsLieux.push({
        nom: l.nom,
        budget: budgetLieuAjuste + bLieu.amortissements + bLieu.interets,
        jours: bLieu.joursAnnuels,
        prixJour: bLieu.joursAnnuels > 0 ? (budgetLieuAjuste + bLieu.amortissements + bLieu.interets) / bLieu.joursAnnuels : 0
      });
    });
  ...
  const exporterExcel = () => {
    ...
    lieux.forEach((lieu, idx) => {
      ...
      lieu.exploitation.forEach(item => {
        csv += `${item.nom},${item.montant},${(item.montant * 12).toFixed(2)}\n`;
      });
    ...
    csv += '\n\n═══════════════════════════════════════════════════════════════════\n';
    csv += 'COMPTE DE RÉSULTAT PRÉVISIONNEL (Budget équilibré)\n';
    csv += '═══════════════════════════════════════════════════════════════════\n\n';
    csv += 'Poste,Année 1,Année 2,Année 3\n';
    
    // Calcul pour chaque année
    const comptesResultat = summary3Ans.map((s, i) => {
      ...
      return {
        produits: s.total,
        salairesBruts: totalSalairesBruts * augmentation,
        chargesSociales: totalChargesSociales * augmentation,
        chargesExternes: totalChargesExploitation * augmentation,
        amortissements: s.amortissements,
        interets: s.interets,
    ...
    csv += `PRODUITS D'EXPLOITATION,${comptesResultat[0].produits.toFixed(2)},${comptesResultat[1].produits.toFixed(2)},${comptesResultat[2].produits.toFixed(2)}\n`;
    csv += '\n';
    csv += `CHARGES D'EXPLOITATION\n`;
    csv += `  Salaires et traitements,${comptesResultat[0].salairesBruts.toFixed(2)},${comptesResultat[1].salairesBruts.toFixed(2)},${comptesResultat[2].salairesBruts.toFixed(2)}\n`;
    csv += `  Charges sociales,${comptesResultat[0].chargesSociales.toFixed(2)},${comptesResultat[1].chargesSociales.toFixed(2)},${comptesResultat[2].chargesSociales.toFixed(2)}\n`;
    csv += `  Autres charges externes,${comptesResultat[0].chargesExternes.toFixed(2)},${comptesResultat[1].chargesExternes.toFixed(2)},${comptesResultat[2].chargesExternes.toFixed(2)}\n`;
    csv += `  Dotations aux amortissements,${comptesResultat[0].amortissements.toFixed(2)},${comptesResultat[1].amortissements.toFixed(2)},${comptesResultat[2].amortissements.toFixed(2)}\n`;
    csv += `  Charges financières (intérêts),${comptesResultat[0].interets.toFixed(2)},${comptesResultat[1].interets.toFixed(2)},${comptesResultat[2].interets.toFixed(2)}\n`;
    csv += '\n';
    csv += `Total Charges,${comptesResultat[0].totalCharges.toFixed(2)},${comptesResultat[1].totalCharges.toFixed(2)},${comptesResultat[2].totalCharges.toFixed(2)}\n`;
    csv += '\n';
    csv += `RÉSULTAT D'EXPLOITATION,${comptesResultat[0].resultat.toFixed(2)},${comptesResultat[1].resultat.toFixed(2)},${comptesResultat[2].resultat.toFixed(2)}\n`;
    
    csv += '\n\nNOTES MÉTHODOLOGIQUES:\n';
    csv += '═══════════════════════════════════════════════════════════════════\n';
    csv += '1. Charges patronales: 42% du salaire brut\n';
    csv += '2. Prime Ségur: 238€/mois chargée à 42% (soit 338€/mois) si applicable\n';
    csv += `3. Taux d'augmentation annuel: ${globalParams.augmentationAnnuelle}% appliqué aux salaires et charges d'exploitation\n`;
    csv += '4. Amortissements: Calcul linéaire (montant / durée), non soumis à augmentation\n';
    csv += '5. Intérêts: Calculés sur le capital total emprunté (simplification), non soumis à augmentation\n';
    csv += '6. Prix de journée: Budget total annuel / Nombre de jours (enfants × taux occupation × 365)\n';
    csv += '7. Budget équilibré: Les produits d\'exploitation sont égaux aux charges (principe du secteur social)\n';
    csv += '\nATTENTION: Le calcul des intérêts est simplifié. Pour un calcul précis des mensualités\n';
    csv += 'et de l\'amortissement du prêt, consulter un expert-comptable.\n';
                  ...
                  <div className="bg-gradient-to-br from-emerald-50 to-teal-50 p-6 rounded-3xl border border-emerald-200">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <button 
                        ...
                        className="bg-emerald-600 text-white p-1.5 rounded-lg hover:bg-emerald-700 transition-all shadow-sm"
                      >
                        <Plus size={16} />
                      </button>
                    </div>
                      ...
                      {lieu.exploitation.map(item => (
                        <div key={item.id} className="flex items-center gap-2 bg-white p-2 rounded-xl shadow-sm group relative">
                          <button 
                            ...
                            className="absolute -top-1 -left-1 bg-red-500 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity z-10"
                          >
                            <Trash2 size={10} />
                          </button>
                          <input 
                            className="flex-1 text-xs font-bold text-slate-600 bg-transparent outline-none border-b border-emerald-100 pb-1"
                            value={item.nom}
